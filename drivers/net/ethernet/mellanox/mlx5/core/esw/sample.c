		   ...
		   struct mlx5_post_action_handle *post_action_handle)
{
	...
	hash_for_each_possible(esw_psample->restore_hashtbl, restore, hlist, hash_key)
		if (restore_equal(restore, obj_id, post_action_handle))
			goto add_ref;
	...
	restore->post_action_handle = post_action_handle;
	...
	hash_add(esw_psample->restore_hashtbl, &restore->hlist, hash_key);
add_ref:
	restore->count++;
			...
			u32 tunnel_id)
{
	...
	if (MLX5_CAP_GEN(esw->dev, reg_c_preserve) ||
		...
		ft = mlx5_post_action_get_ft(esw_psample->post_action);
		default_tbl_id = ft->id;
		post_action_handle = mlx5_post_action_add(esw_psample->priv,
							  esw_psample->post_action,
							  attr);
	...
	sample_flow->sampler = sampler_get(esw_psample, esw_attr->sample->rate, default_tbl_id);
	...
	sample_flow->restore = sample_restore_get(esw_psample, obj_id, post_action_handle);
...
err_post_rule:
	if (post_action_handle)
		mlx5_post_action_del(esw_psample->priv, esw_psample->post_action,
				     post_action_handle);
err_post_action:
	kfree(sample_flow);
	return ERR_PTR(err);
...
struct mlx5_esw_psample *
mlx5_esw_sample_init(struct mlx5e_priv *priv, struct mlx5_post_action *post_action)
{
	...
	if (IS_ERR_OR_NULL(post_action)) {
		err = PTR_ERR(post_action);
		goto err_post_action;
	}
	esw_psample->post_action = post_action;
