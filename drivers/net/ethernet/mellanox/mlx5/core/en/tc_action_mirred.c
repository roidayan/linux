			     ...
			     int act_index)
{
	...
	if (!parse_state->encap && !netdev_port_same_parent_id(priv->netdev, out_dev)) {
...
static int
parse_mirred_encap(struct mlx5e_tc_action_parse_state *parse_state,
		   const struct flow_action_entry *act,
		   struct mlx5_flow_attr *attr)
{
...
static int
parse_mirred(struct mlx5e_tc_action_parse_state *parse_state,
	     const struct flow_action_entry *act,
...
struct mlx5e_tc_action mlx5e_tc_action_mirred = {
	.can_offload = tc_action_can_offload_mirred,
	.parse_action = tc_action_parse_mirred,
};
