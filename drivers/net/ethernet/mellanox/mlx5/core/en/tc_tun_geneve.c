					       ...
					       struct tc_cls_flower_offload *f)
{
	 ...
	 */
	if (be16_to_cpu(key->dst) != GENEVE_UDP_PORT) {
					 ...
					 struct tc_cls_flower_offload *f)
{
	...
	if (dissector_uses_key(f->dissector, FLOW_DISSECTOR_KEY_ENC_KEYID)) {
		...
		MLX5_SET(fte_match_set_misc, misc_c, geneve_vni, be32_to_cpu(mask->keyid));
		MLX5_SET(fte_match_set_misc, misc_v, geneve_vni, be32_to_cpu(key->keyid));
	}
					     ...
					     struct tc_cls_flower_offload *f)
{
	...
	if (!dissector_uses_key(f->dissector, FLOW_DISSECTOR_KEY_ENC_OPTS))
		return 0;
	...
	if (memchr_inv(&mask->data, 0, sizeof(mask->data)) &&
	...
	if (key->dst_opt_type != TUNNEL_GENEVE_OPT) {
		NL_SET_ERR_MSG_MOD(extack,
	...
	if ((key->len / 4) > ((max_tlv_option_data_len + 1) * max_tlv_options)) {
		NL_SET_ERR_MSG_MOD(extack,
	...
	MLX5_SET(fte_match_set_misc, misc_c, geneve_opt_len, mask->len / 4);
	MLX5_SET(fte_match_set_misc, misc_v, geneve_opt_len, key->len / 4);

	/* we support matching on one option only, so just get it */
	option_key = (struct geneve_opt *)&key->data[0];
	option_mask = (struct geneve_opt *)&mask->data[0];
