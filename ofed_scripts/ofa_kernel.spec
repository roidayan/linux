...
install -m 0755 $RPM_BUILD_DIR/%{_name}-%{_version}/ofed_scripts/cx_virt_config.py $RPM_BUILD_ROOT/sbin
%endif
...
#       if [ -f /etc/SuSE-release ]; then
#               if ! ( /sbin/insserv -r mlx4_vnic_confd > /dev/null 2>&1 ); then
#                       true
#               fi
#       fi
       
#       if [ -f /etc/debian_version ]; then
#               if ! ( /usr/sbin/update-rc.d mlx4_vnic_confd remove > /dev/null 2>&1 ); then
#                       true
#               fi
#       fi
