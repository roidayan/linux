...
if [ $with_mlx5 -eq 1 ]; then
	modules="$modules drivers/net/ethernet/mellanox/mlx5/core/mlx5_core"
else
...
echo "MAKE=\"./scripts/mlnx_en_patch.sh ${MLNX_EN_PATCH_PARAMS} && make -j\$(grep ^processor /proc/cpuinfo | wc -l)\""
