	...
	if (rpm -qa 2> /dev/null | grep -q "mlnx-en" || dpkg --list 2> /dev/null | grep -q "mlnx"); then
		...
		if [ $RC == 1 ]; then
			...
			if ( grep -E "Ubuntu|Debian" /etc/issue > /dev/null 2>&1); then
				...
				rpm -e `rpm -qa 2>/dev/nulll | grep -E "mstflint|mlnx.en"` > /dev/null
			fi
		else
...
if [ "$dist_rpm" == "ubuntu" ] || [ "$dist_rpm" == "debian" ]; then
	...
	# RHEL
	if [[ "$distro" =~ "rhel7" && ! "$KVERSION" =~ "el7" ]] ||
	   [[ "$distro" =~ "rhel6" && ! "$KVERSION" =~ "el6" ]]; then
		kmp=0
	fi
	# SLES
	if [[ "$distro" == "sles12sp0" && ! "$KVERSION" =~ 3.12.2[0-9]-[0-9].*-* ]] ||
	   [[ "$distro" == "sles11sp3" && ! "$KVERSION" =~ 3.0.7[6-9]-[0-9].[0-9]*|3.0.[8-9][0-9]-[0-9].[0-9]*|3.0.[1-9][0-9][0-9]-[0-9].[0-9]* ]] ||
